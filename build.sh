...
gcc "$TMP_C" "$RUNTIME_DIR/runtime.c" "$RUNTIME_DIR/sqlite3.c" -o "$OUTPUT" \
    -I "$RUNTIME_DIR" -lm -O2 $STACK_FLAGS $SQLITE_FLAGS
rm "$TMP_C"
...
if [ -f "$LSP_SRC" ]; then
    ...
    gcc "$TMP_LSP_C" "$RUNTIME_DIR/runtime.c" "$RUNTIME_DIR/sqlite3.c" -o "./a-lsp" \
        -I "$RUNTIME_DIR" -lm -O2 $STACK_FLAGS $SQLITE_FLAGS
    rm "$TMP_LSP_C"
