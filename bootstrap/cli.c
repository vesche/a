...
AValue fn__gcc(AValue c_path, AValue out_path, AValue runtime_dir) {
    AValue runtime_c = {0}, sqlite_c = {0}, miniz_c = {0}, include_flag = {0}, cmd = {0}, result = {0};
    AValue __ret = a_void();
    ...
    { AValue __old = cmd; cmd = a_str_join(a_array_new(10, a_string("gcc"), c_path, runtime_c, sqlite_c, miniz_c, a_string("-o"), out_path, include_flag, fn__gcc_flags(), fn__sqlite_flags()), a_string(" ")); a_release(__old); }
...
AValue fn_cmd_test(AValue test_dir) {
    ...
    { AValue __old = sflags; sflags = fn__sqlite_flags(); a_release(__old); }
    {
        ...
        for (int __fi = 0; __fi < a_ilen(__iter_arr); __fi++) {
            ...
            { AValue __old = gcc_result; gcc_result = a_exec(a_str_join(a_array_new(10, a_string("gcc"), c_path, a_str_concat(runtime_dir, a_string("/runtime.c")), a_str_concat(runtime_dir, a_string("/sqlite3.c")), a_str_concat(runtime_dir, a_string("/miniz.c")), a_string("-o"), bin_path, a_str_concat(a_string("-I "), runtime_dir), flags, sflags), a_string(" "))); a_release(__old); }
            a_fs_rm(c_path);
